/**
 * SQLite action - store payment in database
 */
...
export class SqliteAction extends BaseAction {
  ...
  constructor(config: ActionConfig) {
    ...
    if (!dbPath) {
      throw new Error('SqliteAction requires "database" config');
    }
    ...
    try {
      const stmt = this.db.prepare(`
        INSERT INTO ${this.table} (
          wallet, type, invoice, description, description_hash,
          preimage, payment_hash, amount, fees_paid,
          created_at, expires_at, settled_at, metadata
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);
      ...
      if (error instanceof Error && error.message.includes('UNIQUE constraint')) {
        return this.success({ skipped: true, reason: 'duplicate' });
      }

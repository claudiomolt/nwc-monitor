...
export class SessionSendAction extends BaseAction {
  ...
  constructor(config: ActionConfig) {
    ...
    this.template = (config.template as string) || 'Payment: {amount_sats} sats - {description}';
    if (!this.sessionId) throw new Error('SessionSendAction requires sessionId');
  }
  ...
  async execute(payment: Payment): Promise<ActionResult> {
    try {
      ...
      const response = await fetch(`${this.gatewayUrl}/api/sessions/${this.sessionId}/messages`, {
        ...
        body: JSON.stringify({ role: 'user', content: message, metadata: { source: 'nwc-monitor', payment_hash: payment.payment_hash, wallet: payment.wallet } }),
      });
